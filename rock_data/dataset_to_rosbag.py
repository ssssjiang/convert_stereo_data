...
def convert_image_to_ros_msg(image_path, timestamp, frame_id):
    """Converts an image to a ROS Image message."""
...
def process_dataset(dataset_path, output_bag_file):
    ...
    try:
        ...
        # gt_pose_csv = os.path.join(dataset_path, 'mav0', 'vicon', 'imu_pose_tum.csv')
        # if os.path.exists(gt_pose_csv):
        #     gt_pose_data = read_csv_file(gt_pose_csv, delimiter=' ')
        #     for row in gt_pose_data:
        #         try:
        #             timestamp = float(row[0])
        #             pose_msg = convert_pose_to_ros_msg(row, timestamp)
        #             if pose_msg:
        #                 bag.write('/gt_pose', pose_msg, rospy.Time.from_sec(timestamp))
        #                 stats['gt_pose'] += 1
        #         except (ValueError, IndexError) as e:
        #             rospy.logwarn(f"Invalid ground truth pose data: {row}, error: {e}")
...
if __name__ == "__main__":
    ...
    process_dataset(args.dataset_path, args.output_bag_file)
