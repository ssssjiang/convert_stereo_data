...
function show_help {
    ...
    echo "  --Tbc-pattern <模式>    查找特定模式的Tbc文件（默认: *-Tbc.txt）"
...
# 查找对应的Tbc文件
function find_matching_Tbc {
    local camchain_path="$1"
    local dir_path=$(dirname "$camchain_path")
    local base_name=$(basename "$camchain_path" -vio-camchain-imucam.yaml)
    
    # 先尝试直接匹配
    local matching_Tbc="$dir_path/${base_name}-Tbc.txt"
    if [ -f "$matching_Tbc" ]; then
        echo "$matching_Tbc"
        return 0
    fi
    
    # 如果没有找到，则在同一目录下查找Tbc文件
    local found_Tbc=$(find "$dir_path" -maxdepth 1 -name "$base_name*-Tbc.txt" | head -n 1)
    if [ ! -z "$found_Tbc" ]; then
        echo "$found_Tbc"
        return 0
    fi
    
    # 如果还是没找到，尝试使用任何Tbc.txt文件
    found_Tbc=$(find "$dir_path" -maxdepth 1 -name "*-Tbc.txt" | head -n 1)
    if [ ! -z "$found_Tbc" ]; then
        echo "$found_Tbc"
        return 0
    fi
...
while read camchain_file; do
    ...
    # 查找对应的Tbc文件
    Tbc_file=$(find_matching_Tbc "$camchain_file")
    ...
    # todo: --template --use_Tbc1 --format --divide_intrinsics --swap
    CONVERT_CMD="python3 $(dirname "$0")/convert_camchain_to_sensor.py --camchain \"$camchain_file\" --Tbc \"$Tbc_file\" --output \"$sensor_output\" $cmd_options"
...
if [ $DRY_RUN -eq 1 ]; then
    ...
    echo "跳过转换（无Tbc文件）: $skipped_files"
fi 